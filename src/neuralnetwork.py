...
class NeuralNetwork:
    ...
    def save_weights(self, filepath):
        """Guarda pesos e biases de todas as camadas."""
        weights_dict = {}
        for i, layer in enumerate(self.layers):
            if hasattr(layer, 'weights'):
                weights_dict[f'layer_{i}_w'] = layer.weights
                weights_dict[f'layer_{i}_b'] = layer.biases
        np.savez(filepath, **weights_dict)
        print(f"Pesos guardados em {filepath}")

    def load_weights(self, filepath):
        """Carrega pesos e biases do ficheiro .npz."""
        data = np.load(filepath)
        for i, layer in enumerate(self.layers):
            if hasattr(layer, 'weights'):
                layer.weights = data[f'layer_{i}_w']
                layer.biases = data[f'layer_{i}_b']
